...
function cancel_principal() 
{
	...
	cargar_lista("modulos/mantenimiento/ciudadano/ciudadano_d.php?pagina=1");	
}
// 20
function subir(valor,id,nom,apell,ced,ema,tel,dir,odir,paro,muni){
		...
		var telf = tel.split("//");
		var bj = telf[0].replace(/\s/g,'');
		var bw = telf[1].replace(/\s/g,'');
		document.getElementById("thabitacion").value = bj;
		document.getElementById("tmovil").value = bw;		
