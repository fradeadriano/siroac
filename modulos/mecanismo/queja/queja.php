			...
			if(isset($_POST["accion"]))
			{
				switch($_POST["accion"])
				{
					case "Registrar": 
						...
						$rssolicitud->sql = "SELECT n_expediente FROM registro_mecanismo WHERE id_mecanismo = 3 AND DATE_FORMAT(fecha_registro,'%Y') = '".date("Y")."' ORDER BY id_registro_mecanismo DESC LIMIT 1";
						$rssolicitud->abrir();
						if($rssolicitud->total_registros > 0)
							{	
								...
								if ($codviejo[4] == $fechanueva)
								{
									...
									$codnuevo= "Q-".$pertenece."-".$nuevovalor."-".date("m")."-".$fechanueva."-OAC-CEA";
								}
								else
								{
						...
						//?calificacion='.$if.'
						/*bitacora*/ 						
							bitacora($_SESSION["usuario"],date("Y-m-d"),date("H:i:s"),"Registro una Queja identificada con n&uacute;mero: $mYId","Se registro una Queja con id: -$mYId- y con los siguientes datos: '".$bti."'.");
						/*bitacora*/						
						$mensaje = 1;
						/* echo '<script language="javascript">window.open("modulos/mecanismo/queja/pdf_denuncia.php?mecanismo='.$mYId.'","Reporte","location=1,scrollbars=0,width=1000,height=700");</script>'; */																			
						echo '<form action="modulos/mecanismo/queja/xls_queja.php" method="get" name="repor_queja" id="repor_queja">
								<input type="hidden" name="mecanismo" id="mecanismo" >
							 </form>';
						echo '<script language="javascript"> document.getElementById("mecanismo").value = "'.$mYId.'"; document.getElementById("repor_queja").target="_blank"; document.repor_queja.submit(); </script>';																			
		...
		<td align="right">
			<table width="100%" align="center" cellpadding="0" cellspacing="0">
				<tr>
					...
					<td class="titulomenu" valign="middle">Registro de Quejas</td>
				</tr>
				<tr>
							...
							<table border="0" cellpadding="0" cellspacing="3" width="100%" id="contenedor">
								<tr>
									<td colspan="6" align="right" valign="middle">
										...
										<img src="images/Find.png" title="Busqueda de Ciudadanos"onclick="displayMessage('modulos/mecanismo/queja/bsq.php','750','500');" style="cursor:pointer"/>
										&nbsp;&nbsp;&nbsp;<img src="images/agre_ciudadano.png" title="Agregar un Nuevo Ciudadano"onclick="displayMessage('modulos/mecanismo/queja/ciudadano.php','750','550');" style="cursor:pointer"/>
										&nbsp;&nbsp;
									</td>
								</tr>
...
$(document).ready(function()
{
	...
	if(valor==1){
		...
		mensaje=acentos('&iexcl;La Queja ha sido registrada Exitosamente!')
