			...
			if(isset($_POST["accion"]))
			{
				switch($_POST["accion"])
				{
					case "Registrar": 
						...
						$rssolicitud = new Recordset();
						$rssolicitud->sql = "SELECT n_expediente FROM registro_mecanismo WHERE id_mecanismo = 4 AND DATE_FORMAT(fecha_registro,'%Y') = '".date("Y")."' ORDER BY id_registro_mecanismo DESC LIMIT 1";
						$rssolicitud->abrir();
						if($rssolicitud->total_registros > 0)
							{	
								...
								if ($codviejo[4] == $fechanueva)
								{
									...
									$codnuevo= "S-".$pertenece."-".$nuevovalor."-".date("m")."-".$fechanueva."-OAC-CEA";
								}
								else
								{
						...
						/*bitacora*/ 
							...
							bitacora($_SESSION["usuario"],date("Y-m-d"),date("H:i:s"),"Registro una Sugerencia identificada con n&uacute;mero: $mYId","Se registro una Sugerencia con id: -$mYId- y con los siguientes datos: '".$bti."'.");
						/*bitacora*/ 						
						//?calificacion='.$if.'
						$mensaje = 1;						
						echo '<form action="modulos/mecanismo/sugerencia/xls_sugerencia.php" method="get" name="repor_sugerencia" id="repor_sugerencia">
								<input type="hidden" name="mecanismo" id="mecanismo" >
							 </form>';
						echo '<script language="javascript"> document.getElementById("mecanismo").value = "'.$mYId.'"; document.getElementById("repor_sugerencia").target="_blank"; document.repor_sugerencia.submit(); </script>';																			
		...
		<td align="right">
			<table width="100%" align="center" cellpadding="0" cellspacing="0">
				<tr>
					...
					<td class="titulomenu" valign="middle">Registro de Sugerencia</td>
				</tr>
				<tr>
							...
							<table border="0" cellpadding="0" cellspacing="3" width="100%" id="contenedor">
								<tr>
									<td colspan="6" align="right" valign="middle">
										...
										<img src="images/Find.png" title="Busqueda de Ciudadanos"onclick="displayMessage('modulos/mecanismo/sugerencia/bsq.php','750','500');" style="cursor:pointer"/>
										&nbsp;&nbsp;&nbsp;<img src="images/agre_ciudadano.png" title="Agregar un Nuevo Ciudadano"onclick="displayMessage('modulos/mecanismo/sugerencia/ciudadano.php','750','550');" style="cursor:pointer"/>
										&nbsp;&nbsp;
									</td>
								</tr>
...
$(document).ready(function()
{
	...
	if(valor==1){
		...
		mensaje=acentos('&iexcl;La Sugerencia ha sido registrada Exitosamente!')
