...
$Id = stripslashes($_GET["mecanismo"]);
	...
	if ($Id!="")
	{
		...
		$rssolicitud->sql = "SELECT DATE_FORMAT(fecha_registro,'%d/%m/%Y') AS fecha_registro, n_expediente, IF(comunicacion=0,'Escrita','Verbal') AS comunicacion, IF(caso=0,'Con Competencia','Sin Competencia') AS caso, 
								...
								parroquia.`parroquia`, municipio.`municipio`, estado.`estado`, registro_mecanismo.`asesoria`, IF(registro_mecanismo.sub_tipo_mecanismo='ac','Asesoria Comun',IF(registro_mecanismo.sub_tipo_mecanismo='djp','Declaracion Jurada Patrimonio','Datos Filiatorios')) AS tipo
		...
		if($rssolicitud->total_registros != 0)
			{	
				...
				$asesoria = $rssolicitud->decodificar($rssolicitud->fila["asesoria"]);																						
...
$objPHPExcel->getProperties()->setCreator("Adriano Frade")
							 ...
							 ->setTitle("Mecanismos Participacion: Datos Filiatorios")
							 ->setSubject("Siroac")
							 ->setDescription("Asesoria")
							 ->setKeywords("Reporte")
							 ->setCategory("Reporte");
...
$objPHPExcel->setActiveSheetIndex(0)
            ...
            ->setCellValue('C7', "ASESORIA (DATOS FILIATORIOS)");						
$objPHPExcel->getActiveSheet()->setTitle('Asesoria');
...
$objPHPExcel->setActiveSheetIndex(0)
            ...
            ->setCellValue('D9', $acen->decodificar("N&deg; Mecanismo:"))
            ->setCellValue('E9', "$n_expediente")
			
			->setCellValue('A10', "Asesoria:")
			->setCellValue('C10', "$asesoria")
...
if($part3==0){
	...
	$objPHPExcel->getActiveSheet()->getStyle('A'.$h.':F'.$h)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_LEFT);	
}
// Personas, Organismos o Instituciones Involucradas en los Hechos ***********************************************************
...
if($rssolicitud->total_registros != 0)
	{
		...
		for($j = 0; $j < $rssolicitud->total_registros; $j++)
		{
			...
			$contenido = $rssolicitud->decodificar($rssolicitud->fila["contenido"]);	
...
header('Content-Disposition: attachment;filename="asesoria_'.$n_expediente.'_'.date('d-m-Y').'.xlsx"');
