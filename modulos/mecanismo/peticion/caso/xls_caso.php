...
$Id = stripslashes($_GET["mecanismo"]);
	...
	{	
		...
		$rssolicitud->sql = "SELECT DATE_FORMAT(fecha_registro,'%d/%m/%Y') AS fecha_registro, n_expediente, IF(comunicacion=0,'Escrita','Verbal') AS comunicacion, IF(caso=0,'Con Competencia','Sin Competencia') AS caso, 
								...
								IF(ISNULL(ciudadano.`otra_direccion`) = TRUE,'No posee',ciudadano.`otra_direccion`) AS otra_direccion, IF(ISNULL(ciudadano.`telf_habitacion`) = TRUE,'No posee',ciudadano.`telf_habitacion`) AS telf_habitacion, ciudadano.`telf_movil`, 
...
$objPHPExcel->getProperties()->setCreator("Adriano Frade")
							 ...
							 ->setTitle("Mecanismos Participacion: Caso")
							 ->setSubject("Siroac")
							 ->setDescription("Caso")
							 ->setKeywords("Reporte")
							 ->setCategory("Reporte");
...
$objPHPExcel->setActiveSheetIndex(0)
            ...
            ->setCellValue('D7', "PETICION - ASESORIA");						
$objPHPExcel->getActiveSheet()->setTitle('Caso');
...
header('Content-Disposition: attachment;filename="caso_'.$n_expediente.'_'.date('d-m-Y').'.xlsx"');
