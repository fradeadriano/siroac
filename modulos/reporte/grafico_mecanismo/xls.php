...
$Id = stripslashes($_POST["condiciones"]);
	...
	if ($Id!="")
	{
		...
		$rssolicitud->sql = "SELECT DATE_FORMAT(fecha_registro,'%d/%m/%Y') AS fecha_registro, n_expediente, IF(comunicacion=0,'Escrita','Verbal') AS comunicacion, IF(caso=0,'Con Competencia','Sin Competencia') AS caso, 
								...
								parroquia.`parroquia`, municipio.`municipio`, estado.`estado`
								
							FROM registro_mecanismo INNER JOIN ciudadano ON registro_mecanismo.`id_ciudadano` = ciudadano.`id_ciudadano` 
											INNER JOIN parroquia ON ciudadano.`id_parroquia` = parroquia.`id_parroquia`
		...
		if($rssolicitud->total_registros != 0)
			{	
				...
				$nombres = $rssolicitud->fila["nombres"];
				...
				$estado = $rssolicitud->fila["estado"];																								
			}
		$rssolicitud->cerrar();
		unset($rssolicitud);
		...
		if($rssolicitud->total_registros != 0)
			{	
				...
				if($rssolicitud->fila["nombre_instancia_pp"]!=""){
					$part1=0;
					$nombre_instancia_pp = $rssolicitud->fila["nombre_instancia_pp"];
					$rif_instancia_pp = $rssolicitud->fila["rif_instancia_pp"];		
					$direccion_instancia_pp = $rssolicitud->fila["direccion_instancia_pp"];
					$ente_financiero = $rssolicitud->fila["ente_financiero"];
					$nombre_proyecto = $rssolicitud->fila["nombre_proyecto"];
					$monto_financiado = $rssolicitud->fila["monto_financiado"];
					$pertenece_comite = $rssolicitud->fila["pertenece_comite"];
					$nombre_comite = $rssolicitud->fila["nombre_comite"];	
				} else {
					$part1=1;				
				}
			}
		...
		if($rssolicitud->total_registros != 0)
			{	
				...
				{
					$part2=0;				
					$nombre_organizacion_sindical = $rssolicitud->fila["nombre_organizacion_sindical"];
					$direccion_organizacion_sindical = $rssolicitud->fila["direccion_organizacion_sindical"];		
		...
		if($rssolicitud->total_registros != 0)
			{	
				...
				{					
					$part3=0;
					$actores_involucrados = $rssolicitud->fila["actores_involucrados"];
				} else {
					$part3=1;
				}
			}
		...
		if($rssolicitud->total_registros != 0)
			{	
				...
				if($rssolicitud->fila["escena_suceso"]!="")
					{
						$part4=0;				
						$escena_suceso = $rssolicitud->fila["escena_suceso"];
					} else {
						$part4=1;				
					}
			}
		...
		if($rssolicitud->total_registros != 0)
			{
				...
				if($rssolicitud->fila["relatoria_caso"]!="")
					{
						$part6=0;				
						$relatoria_caso = $rssolicitud->fila["relatoria_caso"];
					} else {
						$part6=1;				
					}
			}
		...
		if($rssolicitud->total_registros != 0)
			{
				...
				if($rssolicitud->fila["observacion"]!="")
					{
						$part7=0;					
						$observacion = $rssolicitud->fila["observacion"];
					} else {
						$part7=1;				
					}
			}
...
$objPHPExcel->getProperties()->setCreator("Adriano Frade")
							 ...
							 ->setTitle("Mecanismos Participacion: Denuncia")
							 ->setSubject("Siroac")
...
$objPHPExcel->getActiveSheet()->mergeCells('D5:D5');	
$objPHPExcel->getActiveSheet()->mergeCells('D8:D8');

$objPHPExcel->setActiveSheetIndex(0)
            ->setCellValue('D4', "CONTRALORIA DEL ESTADO ARAGUA")
            ->setCellValue('D5', "OFICINA DE ATENCION CIUDADANA")
            ->setCellValue('D7', "DENUNCIA");						
...
$objPHPExcel->setActiveSheetIndex(0)
            ...
            ->setCellValue('E17', "Municipio:")
			->setCellValue('F17', "$municipio");
...
$objPHPExcel->setActiveSheetIndex(0)
            ->setCellValue('A18', "Estado:")
			->setCellValue('C18', "$estado")
            ->setCellValue('E18', "Correo:")
			->setCellValue('F18', "$correo");
$objPHPExcel->getActiveSheet()->getStyle('A18')->getFont()->setBold(true);			
$objPHPExcel->getActiveSheet()->getStyle('E18')->getFont()->setBold(true);			
$objPHPExcel->getActiveSheet()->getStyle('A18')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);
$objPHPExcel->getActiveSheet()->getStyle('E18')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);
...
$objPHPExcel->setActiveSheetIndex(0)
            ->setCellValue('A19', "Otra Direccion:")
			->setCellValue('C19', "$otra_direccion");
...
$objPHPExcel->getActiveSheet()->getStyle('C19')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_LEFT);
...
if($part1==0){
	...
	$objPHPExcel->getActiveSheet()->getCell('C'.$h)->setValueExplicit($monto_financiado, PHPExcel_Cell_DataType::TYPE_STRING);
	$objPHPExcel->getActiveSheet()->mergeCells('A'.$h.':B'.$h);
...
if($rssolicitud->total_registros != 0)
	{
		...
		$objPHPExcel->getActiveSheet()->mergeCells('E'.$h.':F'.$h);
		$objPHPExcel->getActiveSheet()->getStyle('A'.$h.':F'.$h)->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID);
		$objPHPExcel->getActiveSheet()->getStyle('A'.$h.':F'.$h)->getFill()->getStartColor()->setARGB('CCCCCC');				
		$objPHPExcel->setActiveSheetIndex(0)
			->setCellValue('A'.$h, "Tipo:")
			->setCellValue('E'.$h, $rssolicitud->decodificar("N&deg; Folio:"));	
		$objPHPExcel->getActiveSheet()->getStyle('A'.$h)->getFont()->setBold(true);			
		$objPHPExcel->getActiveSheet()->getStyle('E'.$h)->getFont()->setBold(true);					
		$objPHPExcel->getActiveSheet()->getStyle('A'.$h)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);
		$objPHPExcel->getActiveSheet()->getStyle('E'.$h)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_LEFT);			
...
//*
			...
			{
				$objPHPExcel->setActiveSheetIndex(0)
							->setCellValue('A'.$h, "Otros:")
							->setCellValue('E'.$h, "$n_folios");			
				$h=$h+1;								
				$objPHPExcel->getActiveSheet()->mergeCells('A'.$h.':C'.$h);
				$objPHPExcel->getActiveSheet()->mergeCells('D'.$h.':F'.$h);
				$objPHPExcel->setActiveSheetIndex(0)
							->setCellValue('D'.$h, "$contenido");			
				
			} else {
				$objPHPExcel->setActiveSheetIndex(0)
							->setCellValue('A'.$h, "$prueba_consignada")
							->setCellValue('E'.$h, "$n_folios");				
			}
			$borders = array(
				  'borders' => array(
...
//PIE
	...
	$rssolicitud = new Recordset();
	$rssolicitud->sql = "SELECT CONCAT(nombres,' ',apellidos) AS nombres FROM seg_usuario WHERE usuario = '".$_SESSION["usuario"]."'";
	$rssolicitud->abrir();
	if($rssolicitud->total_registros != 0)
		{
			$rssolicitud->siguiente();
			$usu = $rssolicitud->fila["nombres"];		
...
header('Content-Disposition: attachment;filename="denuncia_'.$n_expediente.'_'.date('d-m-Y').'.xlsx"');
